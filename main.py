...
df_score_init = pd.DataFrame(score_data, index=quarters)
edited_score_df = st.data_editor(df_score_init, use_container_width=True)
...
for i in range(9):
    blue_score = edited_score_df.iloc[i]["블루 점수"]
    black_score = edited_score_df.iloc[i]["블랙 점수"]
    red_score = edited_score_df.iloc[i]["레드 점수"]
    ...
    if i % 3 == 0:
        ...
        t_pairs = ["ブル", "블랙"]; scores = [int(blue_score), int(black_score)]
